...
esp_err_t deb_btns_update(deb *btns, uint8_t btns_count)
{
	...
	for (uint8_t i = 0; i < btns_count; i++)
	{
		...
		if (btns[i].pinState == PRESSED)
		{
			...
			// Перевірка 2: Повторення події (Auto-repeat), якщо кнопка продовжує утримуватися
			else
			{
				// Оптимізація: рахуємо інтервал безпосередньо від МИНУЛОГО ПОВТОРУ
				// Це захищає від переповнення heldTime при критично довгому утриманні
				if (now - btns[i].lastRepeatTime >= btns[i].repeatInterval)
				{
					btns[i].lastRepeatTime = now;
					btns[i].event = LONG_PRESS_REPEAT_EVENT; // Генеруємо подію повтору
				}
			}
		}
	}
